...
class Memory(object):
    ...
    def update(self, nids: Union[np.ndarray, Tensor], newdata: Tensor, newtime: Tensor):
        ...
        nids = nids.to(self._device, non_blocking=True)

        data_src = newdata.detach()
        if data_src.device != self._device:
            data_src = data_src.to(self._device, non_blocking=True)

        time_src = newtime.detach()
        if time_src.device != self._device:
            time_src = time_src.to(self._device, non_blocking=True)

        self._data.index_copy_(0, nids, data_src)
        self._time.index_copy_(0, nids, time_src)
