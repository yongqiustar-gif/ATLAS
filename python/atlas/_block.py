...
class TBlock(object):
    ...
    def __init__(self, g: 'TGraph', layer: int, dstnodes: np.ndarray, dsttimes: np.ndarray,
        ...
        :param np.ndarray dstnodes: The destination nodes. (for layer 0, these are the [src, dst, neg])
        :param np.ndarray dsttimes: The destination node timestamps. (for layer 0, these are the [times, times, times])
    ...
    def next_block(self, include_dst=False, use_dst_times=False) -> TBlock:
        ...
        if self._next is not None:
            return self._next
        # t_start = tt.start()
        nvtx.range_push("prepare next block")
        if self.num_dst() == 0:
            blk = TBlock(self._g, self._layer + 1, np.empty(0), np.empty(0))
        else:
            ...
            blk = TBlock(self._g, self._layer + 1, next_dstnodes, next_dsttimes)
        blk._include_prev_dst = True
        blk._prev = self
        self._next = blk
        nvtx.range_pop()  # prepare next block
        # tt.t_prep_input += tt.elapsed(t_start)
        return blk

    
    def allnodes(self) -> np.ndarray:
        """Returns a numpy array containing the destination nodes concatenated with the source nodes (if available)."""
        return np.concatenate([self._dstnodes, self._srcnodes]) if self._has_nbrs else self._dstnodes

    
    def uniq_src(self) -> Optional[Tuple[np.ndarray, np.ndarray]]:
        """Returns a numpy array tuple: (the unique source node indices, the indices of the unique array that reconstructs the source
         node array)."""
        if self._srcnodes is None:
            return None
        uniq_nids, idx = np.unique(self._srcnodes, return_inverse=True)
        return uniq_nids, idx
    ...
    def _load_efeat(self):
        ...
        if self._c_efeat is None and self._g.efeat is not None:
            self._c_efeat = self._load_feat(
                self._g.efeat, self._eid)

    def _load_nfeat(self):
        ...
        if self._c_nfeat is None and self._g.nfeat is not None:
            self._c_nfeat = self._load_feat(
                self._g.nfeat, self.allnodes())

    def _load_feat(self, feat: Tensor, idx: np.ndarray) -> Tensor:
