...
sample_layer(TCSR &tcsr, TemporalBlock &block,
  ...
  #pragma omp parallel
  {
    ...
    for (size_t j = 0; j < size; j++) {
      ...
      IdI32 e_bound = tcsr.ind[nid + 1];
      int added_count = 0;

      if (s_search < e_bound) {
        auto e_it = std::lower_bound(tcsr.ets.begin() + s_search,
                                     tcsr.ets.begin() + e_bound, nts);
        IdI32 e_search = static_cast<IdI32>(e_it - tcsr.ets.begin()) - 1;

        if (e_search >= s_search) {
      ...
      // Padding logic
      while (added_count < _num_nbrs) {
        add_padding(eid[tid], ets[tid], srcnodes[tid], dstindex[tid], out_nodes[tid]);
        added_count++;
      }
...
inline void FixedTemporalSampler::
...
add_padding(
    ...
    std::vector<IdI32> *srcnodes, std::vector<IdI32> *dstindex,
    IdI32 &dst_idx) {
  eid->push_back(-1);       // No edge
  ets->push_back(0.0f);     // Zero timestamp
  srcnodes->push_back(-1);  // Padding node index
  dstindex->push_back(dst_idx);
}
