...
class AsyncPipelineTrainer:
    ...
    def __init__(
        ...
        self.cg_capture_total_ms: float = 0.0
        self.cg_num_templates: int = 0
        self.cg_extra_mem_bytes: int = 0
        self.cg_total_mem_bytes: int = 0
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.epochs):
            ...
            print(f"{idx} - STEADY_BEGIN_MS {int(time.time()*1000)}", flush=True)
            t_epoch = tt.start()
            self.model.train()
            ...
            print(f"{idx} - STEADY_END_MS {int(time.time()*1000)}", flush=True)
            idx +=1
    ...
    def _prepare_cuda_graph(self) -> None:
        ...
        mem_before = torch.cuda.memory_reserved(self.device)
        total_mem = torch.cuda.get_device_properties(self.device).total_memory

        with _nvtx_range("cg_template_build"):
        ...
        with _nvtx_range("cg_template_h2d"):
        ...
        with _nvtx_range("cg_warmup_total"):
        ...
        with _nvtx_range("cg_capture_total"):
        ...
        self.cg_capture_total_ms = (time.perf_counter() - cap_start) * 1000.0
        self.cg_num_templates = 1
        mem_after = torch.cuda.memory_reserved(self.device)
        self.cg_extra_mem_bytes = max(0, int(mem_after - mem_before))
        self.cg_total_mem_bytes = int(total_mem)

        cap_per_tpl = self.cg_capture_total_ms / max(1, self.cg_num_templates)
        extra_mem_mb = self.cg_extra_mem_bytes / (1024.0 * 1024.0)
        extra_mem_pct = (self.cg_extra_mem_bytes / self.cg_total_mem_bytes * 100.0) if self.cg_total_mem_bytes > 0 else 0.0
        print(
            "cudagraph_cost: "
            f"capture_total_ms={self.cg_capture_total_ms:.3f} "
            f"num_tpl={self.cg_num_templates} "
            f"capture_per_tpl_ms={cap_per_tpl:.3f} "
            f"extra_mem_mb={extra_mem_mb:.3f} "
            f"extra_mem_pct={extra_mem_pct:.6f}"
        )
    ...
    def _train_on_block(self, gpu_block: at.AtlasGPUBlock) -> float:
        if (
            ...
            with _nvtx_range("cg_replay_copy"):
